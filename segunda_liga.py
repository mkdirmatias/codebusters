...
# clase atacar (stun)
class Atacar(Funcion):
    def accion(self):
        # verificar que no esté congelado
        if self.entity.congelado == 0:
            # recorremos los cazadores que no están en mi equipo
            for entity_id, cazador in cazadores[not self.entity.id_team].items():
                # verificamos que esté dentro de nuestro alcance
                if not cazador.fuera_alcance:
                    # y que esté dentro de la distancia para poder stunear
                    # y que mi cazador no esté con un fantasma (ocupado)
                    if Distancia(self.entity, cazador) < 1700 and not self.entity.ocupado:
                        # cambiamos el estado de congelado
                        self.entity.congelado = 20
                        # y stuneamos al enemigo
                        return Stun(entity_id)

        # volvemos a asignar una tarea
        return self.funcion_default.accion()
